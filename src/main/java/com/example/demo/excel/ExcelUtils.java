...
public class ExcelUtils {
    ...
    //手机号正则表达式
    private static final Pattern PHONE_PATTERN = Pattern.compile("^(?:(?:\\+|00)86)?(13[0-9]|14[01456789]|15[0-35-9]|17[0-35-8]|18[0-35-9]|19[0-35-9])\\d{8}$");
    //身份证号正则表达式
    private static final Pattern ID_CARD_PATTERN = Pattern.compile("^\\d{6}((((((19|20)\\d{2})(0[13-9]|1[012])(0[1-9]|[12]\\d|30))|(((19|20)\\d{2})(0[13578]|1[02])31)|((19|20)\\d{2})02(0[1-9]|1\\d|2[0-8])|((((19|20)([13579][26]|[2468][048]|0[48]))|(2000))0229))\\d{3})|((((\\d{2})(0[13-9]|1[012])(0[1-9]|[12]\\d|30))|((\\d{2})(0[13578]|1[02])31)|((\\d{2})02(0[1-9]|1\\d|2[0-8]))|(([13579][26]|[2468][048]|0[048])0229))\\d{2}))(\\d|X|x)$");
    ...
    private static <T> void setFieldValue(T t, Field field, JSONObject obj, StringBuilder uniqueBuilder, List<String> errMsgList) {
        ...
        if (minLength > 1 && val.length() < minLength) {
            errMsgList.add(String.format("[%s]长度必须达到%s个字符(当前%s个字符)", cname, minLength, val.length()));
        }

        //判断长度
        int length = annotation.length();
        if (length > 1 && val.length() != length) {
            errMsgList.add(String.format("[%s]长度必须是%s个字符(当前%s个字符)", cname, length, val.length()));
        }

        //判断日期是否在今天之前
        boolean b = annotation.beforeNow();
        if (b) {
            LocalDateTime now = LocalDateTime.now();
            LocalDateTime localDateTime = LocalDateTime.parse(val, DATE_TIME_FORMATTER);
            if (localDateTime.isAfter(now)) {
                errMsgList.add(String.format("[%s]必须在合理范围内", cname));
            }
        }
        //判断手机号是否符合规范
        boolean phoneNumber = annotation.isPhoneNumber();
        if (phoneNumber) {
            Matcher matcher = PHONE_PATTERN.matcher(val);
            if (!matcher.matches()) {
                errMsgList.add(String.format("[%s]格式错误,不符合规范", cname));
            }
        }
        //判断身份证是否符合规范
        boolean idCard = annotation.isIdCard();
        if (idCard){
            Matcher matcher = ID_CARD_PATTERN.matcher(val);
            if (!matcher.matches()){
                errMsgList.add(String.format("[%s]格式错误,不符合规范", cname));
            }
        }
        ...
        try {
            if ("String".equalsIgnoreCase(fieldClassName)) {
                ...
                if (!val.equals("") && val != null) {
                    field.set(t, LocalDateTime.parse(val, DATE_TIME_FORMATTER));
                } else {
                    field.set(t, null);
                }
            }else if("LocalDate".equalsIgnoreCase(fieldClassName)){
                if (!val.equals("") && val != null) {
                    LocalDateTime parse = LocalDateTime.parse(val, DATE_TIME_FORMATTER);
                    field.set(t, parse.toLocalDate());
                } else {
                    field.set(t, null);
                }
            }
    ...
    private static String getCellValue(Cell cell) {
        ...
        if (cell.getCellTypeEnum() == CellType.NUMERIC) {
            if (HSSFDateUtil.isCellDateFormatted(cell)) {
                ...
                String format = SIMPLE_DATE_FORMAT.format(dateCellValue);
                return format + "";
            }
            // 科学计数法类型
    ...
    private static int setCellValue(Cell cell, Object o, CellStyle style) {
        ...
        if (o instanceof LocalDateTime) {
            ...
            cell.setCellType(CellType.STRING);
            //转换为localDateTime类型
            LocalDateTime localDateTime = Convert.toLocalDateTime(o);
